...
void test_dma_writer(DMA_DEMO_WRITER * w){
  ...
  printf ("DMA Buffer Size: Hex: 0x%08X, Dec: %d\n",  w->get_buffer_size(),
                                                    w->get_buffer_size());
  printf ("DMA Written Size: Hex: 0x%08X, Dec: %d\n", w->get_written_size(), 
                                                    w->get_written_size());
...
void test_lcd(Nysa *nysa, uint32_t dev_index, bool debug){
  ...
  uint32_t  cyan   = 0x00FFF000;
  uint32_t  purple = 0xF000FF00;
  uint32_t  orange = 0xFFF00000;
  uint32_t  pink   = 0xFF707000;
  uint32_t  black  = 0x00000000;

  uint32_t size = 0;

  //NH_LCD_480_272 * lcd = new NH_LCD_480_272(nysa, dev_index, debug);
  NH_LCD_480_272 * lcd = new NH_LCD_480_272(nysa, dev_index, true);
  //Create the color buffers
  size = lcd->get_buffer_size();
  printf ("Size: Hex: 0x%08X, Dec: %d\n", size, size);
  ...
  for (int i = 0; i < size; i += 4){
  //for (int i = 0; i < size; i += 4){
    buffer[2][i + 0] = (uint8_t) ((purple >> 24) & 0xFF);
    buffer[2][i + 1] = (uint8_t) ((purple >> 16) & 0xFF);
    buffer[2][i + 2] = (uint8_t) ((purple >>  8) & 0xFF);
    buffer[2][i + 3] = (uint8_t) ((purple      ) & 0xFF);
  }
  //Green
  for (int i = 0; i < size; i += 4){
  //for (int i = 0; i < size; i += 4){
    buffer[3][i + 0] = (uint8_t) ((blue >> 24) & 0xFF);
    buffer[3][i + 1] = (uint8_t) ((blue >> 16) & 0xFF);
    buffer[3][i + 2] = (uint8_t) ((blue >>  8) & 0xFF);
    buffer[3][i + 3] = (uint8_t) ((blue      ) & 0xFF);
  }


  //Working
  //uint32_t sleep_timeout = 100000;
  uint32_t sleep_timeout = 10000;
  for (int i = 0; i < 10; i++){
    //Write the LCD with the first color
    //printf ("Writing Images 0 (Red)...\n");
    lcd->dma_write(buffer[0]);
    usleep(sleep_timeout);
 
    //printf ("Writing Images 1 (Cyan)...\n");
 
    //Write the LCD with the second color
    lcd->dma_write(buffer[1]);
    usleep(sleep_timeout);
    //printf ("Writing Images 2 (Purple)...\n");
    //Write the LCD with the third color
 
 
    lcd->dma_write(buffer[2]);
    usleep(sleep_timeout);
    //printf ("Writing Images 3 (Green)...\n");
...
int main(int argc, char **argv){
  ...
  Dionysus * dionysus = new Dionysus(args.debug);
  ...
  if (dionysus->is_open()){
    ...
    //LCD Color Test
    device = dionysus->find_device(get_lcd_type(), get_nh_lcd_480_272_type());
    if (device > 0){
      if (args.lcd_test){
        printf ("Found an LCD!\n");
